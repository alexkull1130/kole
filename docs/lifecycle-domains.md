...
The next runtime slice will add an explicit owner close operation and owner-bound callback registration. Until then, a domain can be modeled with a `Closeable` owner method that closes its owned resources in reverse declaration order. Native bindings should expose the same rule instead of asking each script to remember cleanup conventions.
