...
Status: executable prototype, version 0.2.0. This document distinguishes implemented behavior from future language goals. `.k`, `name: Type`, `method() -> Type`, and the explicit `for(i=0:10:+)` form are settled requirements; endpoint and counter rules below are provisional implementation decisions. See [requirements](requirements.md) for the full roadmap.
...
Fields belong to individual objects. Methods can access their own private fields and private methods, including those on other instances of the same class. External classes cannot. Static methods have no `this`. Static fields, inheritance, interfaces, imports, and packages are not implemented yet.

Variables, fields, parameters, and returns carry declared types. The checker validates names, member access, assignments, call arguments, operators, condition types, and return types and paths before execution. Enum types retain their declaring class identity. Runtime type checks remain as a backstop. Definite-assignment and constructor initialization analysis are not implemented yet: reading a field or variable without an initializer or preceding assignment still raises a runtime error.

Supported value types are signed 32-bit `int`, finite numeric `double`, `boolean`, `String`, declared classes, and enums declared within a class. Primitive and class array type annotations are parsed, but array creation and mutation are not implemented; the command-line argument array can be read. Null references are allowed for String and class types in this prototype. Non-null-by-default semantics remain future work.
...
`check` validates all initializers and bodies without executing user code. It reports the first error and does not require an entrypoint, allowing library-style source files to be checked. `run` performs the same checks before running the entrypoint. Return-path analysis is conservative for range loops, which may be empty. Initialization, null safety, arithmetic/index safety, and lifecycle correctness remain runtime concerns. Direct low-level `Runtime` use is available for interpreter testing; callers that want preflight checking should use the public `run` helper or call `check` explicitly.

## Not implemented

Ownership/relationship keywords (`owns`, `belongsTo`), atomic blocks, type inference, definite-assignment analysis, static lifecycle analysis, null safety, immutable-by-default values, data classes, properties, sealed types/pattern matching, generics, inheritance/interfaces, exceptions with user-defined throw/catch, modules, collection libraries, custom loop steps, concurrency, optional memory control, and native code generation. They must not be inferred from the broader design sketches in the README.
